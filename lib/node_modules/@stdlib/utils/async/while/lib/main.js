...
function whileAsync(predicate, fcn, done, thisArg) {
	...
	if (!isFunction(predicate)) {
		throw new TypeError(
			format(
				"invalid argument. First argument must be a function. Value: `%s`.",
				predicate,
			),
		);
	}
	if (!isFunction(fcn)) {
		throw new TypeError(
			format(
				"invalid argument. Second argument must be a function. Value: `%s`.",
				fcn,
			),
		);
	}
	if (!isFunction(done)) {
		throw new TypeError(
			format(
				"invalid argument. Third argument must be a function. Value: `%s`.",
				done,
			),
		);
	}
